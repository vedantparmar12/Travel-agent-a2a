"""
A2A Agent Executor for Transport Agent.
"""
import json
import uuid
...
from .transport_agent_a2a import TransportAgentA2A
    ...
    async def invoke(
        ...
        try:
            ...
            async for update in self.agent.stream(query, context_id):
                if update.get("is_task_complete"):
                    ...
                    artifact = TaskArtifact(
                        id=str(uuid.uuid4()),
                        type="text",
                        title="Transport Search Results",
            ...
            error_artifact = TaskArtifact(
                ...
                title="Error",
                artifact=TextArtifact(
                    parts=[TextPart(text=f"Error processing transport request: {str(e)}")]
                ),
            )
