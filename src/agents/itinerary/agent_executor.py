"""
A2A Agent Executor for Itinerary Agent.
"""
import json
import uuid
...
from .itinerary_agent_a2a import ItineraryAgentA2A


class ItineraryAgentExecutor(AgentExecutor):
    """A2A executor for Itinerary Agent."""
    
    def __init__(self):
        self.agent = ItineraryAgentA2A()
    ...
    async def invoke(
        ...
        try:
            ...
            error_artifact = TaskArtifact(
                ...
                title="Error",
                artifact=TextArtifact(
                    parts=[TextPart(text=f"Error processing itinerary request: {str(e)}")]
                ),
            )
