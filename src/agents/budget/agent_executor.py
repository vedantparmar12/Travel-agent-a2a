"""
A2A Agent Executor for Budget Agent.
"""
import json
import uuid
...
from .budget_agent_a2a import BudgetAgentA2A


class BudgetAgentExecutor(AgentExecutor):
    """A2A executor for Budget Agent."""
    
    def __init__(self):
        self.agent = BudgetAgentA2A()
    ...
    async def invoke(
        ...
        try:
            ...
            error_artifact = TaskArtifact(
                ...
                title="Error",
                artifact=TextArtifact(
                    parts=[TextPart(text=f"Error processing budget request: {str(e)}")]
                ),
            )
