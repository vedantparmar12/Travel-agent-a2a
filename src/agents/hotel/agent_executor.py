"""
A2A Agent Executor for Hotel Agent.
"""
import json
import uuid
...
from .hotel_agent_a2a import HotelAgentA2A


class HotelAgentExecutor(AgentExecutor):
    """A2A executor for Hotel Agent."""
    
    def __init__(self):
        self.agent = HotelAgentA2A()
    ...
    async def invoke(
        ...
        # Extract context ID from message
        ...
        try:
            ...
            async for update in self.agent.stream(query, context_id):
                if update.get("is_task_complete"):
                    ...
                    # Optionally send status artifact
